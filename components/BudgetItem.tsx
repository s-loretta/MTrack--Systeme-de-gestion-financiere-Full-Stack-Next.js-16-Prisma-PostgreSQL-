...
const BudgetItem : React.FC<BudgetItemProps>= ({budget, enableHover}) => {
  ...
  return (
    <li key={budget.id} className={`p-4 rounded-xl border-base-300 list-none border-2 ${hoverClasse}`}>
        ...
        <div className='flex justify-between items-center mx-4 text-gray-500 text-sm'>
             <span> {totalTransactionAmount} € dépensé </span>
            <span> {remainingAmount} € restant </span>
        </div>

        <div> 
            <progress className="progress progress-secondary w-full mt-4" value={totalTransactionAmount} max={budget.amount}></progress>
        </div>
